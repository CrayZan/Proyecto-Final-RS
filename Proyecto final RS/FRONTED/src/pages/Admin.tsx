...
import { ArrowLeft, Plus, Trash2, Upload, Edit, Save, X, FileText } from "lucide-react"
  ...
  return (
    <Dialog open={abierto} onOpenChange={setAbierto}>
      <DialogTrigger asChild>
        <Button variant="outline" size="icon" className="h-8 w-8 text-slate-600 border-slate-200"><Edit className="h-4 w-4" /></Button>
      </DialogTrigger>
      <DialogContent>
        ...
        <div className="py-4 space-y-2">
          ...
          <Input type="number" value={precioTemporal} onChange={(e) => setPrecioTemporal(e.target.value)} className="text-lg font-bold" />
        </div>
        <DialogFooter>
          <Button onClick={() => { alGuardar(producto.id, Number(precioTemporal)); setAbierto(false); }} className="bg-slate-900 w-full h-12 font-bold uppercase">Actualizar</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export default function Admin({ productos, setProductos }: { productos: any[], setProductos: any }) {
  ...
  const [nuevaDescripcion, setNuevaDescripcion] = useState("") // <-- ESTADO PARA DESCRIPCIÓN
  ...
  const agregarProducto = () => {
    ...
    const nuevo = { 
      ...
      descripcion: nuevaDescripcion, // <-- GUARDAMOS DESCRIPCIÓN
      categoria: nuevaCat,
    ...
    toast.success("Producto creado con descripción")
  }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-black text-slate-900 uppercase">Panel de Control</h1>
        <Dialog open={modalNuevoAbierto} onOpenChange={setModalNuevoAbierto}>
          <DialogTrigger asChild><Button className="bg-orange-600 h-12 px-6 font-bold uppercase shadow-lg"><Plus className="mr-2 h-5 w-5" /> Nuevo Producto</Button></DialogTrigger>
          <DialogContent className="max-w-md">
            ...
            <div className="grid gap-4 py-4 text-left">
              ...
              <div className="grid gap-2">
                ...
                <Textarea 
                  placeholder="Ej: Jamón, morrones, aceitunas..." 
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-slate-900"><TableRow>
              <TableHead className="text-white text-xs uppercase font-black">Producto</TableHead>
              ...
              {productos.map((p: any) => (
                <TableRow key={p.id} className="hover:bg-slate-50/50">
                  <TableCell className="flex gap-4 items-center py-4">
                    ...
                    <div>
                      <div className="font-black text-slate-800 uppercase text-sm">{p.nombre}</div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase max-w-[200px] truncate">{p.descripcion || "Sin descripción"}</div>
                      <div className="text-orange-600 font-black text-xs mt-1">${p.precio.toLocaleString('es-AR')}</div>
                    </div>
                  </TableCell>
                  <TableCell className="text-right pr-6">
                    <div className="flex justify-end gap-2">
                      <ModalEditar producto={p} alGuardar={actualizarPrecio} />
                      <Button variant="outline" size="icon" className="h-8 w-8 text-red-400 border-slate-200" onClick={() => setProductos(productos.filter((i:any) => i.id !== p.id))}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
